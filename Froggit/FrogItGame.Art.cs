...
namespace Froggit
{
    public partial class FrogItGame
    {
        ...
        void InitBuffers()
        {
            ...
            frogUp = LoadSprite(frogU);
            frogLeft = LoadSprite(frogL);
            frogRight = LoadSprite(frogR);
            ...
            logDarkLeft = LoadSprite(logDarkL);
            logDarkCenter = LoadSprite(logDarkC);
            logDarkRight = LoadSprite(logDarkR);
            ...
            crocLeft = LoadSprite(crocL);
            crocCenter = LoadSprite(crocC);
            crocRight = LoadSprite(crocR);
            ...
            truckLeft = LoadSprite(truckL);
            truckCenter = LoadSprite(truckC);
            truckRight = LoadSprite(truckR);

            byte[] carL = { 0x1c, 0x3f, 0x4c, 0x4c, 0x4c, 0x3f, 0x1c, 0x00 };
            byte[] carR = { 0x1c, 0xfe, 0x71, 0x71, 0x71, 0xfe, 0x1c, 0x00 };

            carLeft = LoadSprite(carL);
            carRight = LoadSprite(carR);
        }

        Buffer1bpp LoadSprite(byte[] data, int width = 8, int height = 8)
        {
            return new Buffer1bpp(width, height, data)
                .RotateAndConvert<Buffer1bpp>(RotationType._90Degrees)
                .ScaleUp<Buffer1bpp>(2);
